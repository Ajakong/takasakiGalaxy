...
void WarpGate::SetEffectPos()
{
	...
	mat = MMult(mat,MGetTranslate(Vec3(m_rigid->GetPos()+m_upVec*800).VGet()));
...
void WarpGate::OnCollideEnter(std::shared_ptr<Collidable> colider)
{
	...
	if (colider->GetTag() == ObjectTag::Player)
	{
		...
		colider->GetRigidbody()->SetVelocity(Vec3(m_warpPos - colider->GetRigidbody()->GetPos()).GetNormalized() * 10);
